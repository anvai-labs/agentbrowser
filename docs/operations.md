...
| A visible form control never appears in observations | The control carries no ARIA role (div-based widget trigger). Observe with `include:["formControls"]` to mint refs for it; act-through works like any ref. See the [interactive forms recipe](recipes/interactive-forms.md). |
