...
`expectValue` makes the engine read the field's value back (one refill) and
fail with `VALUE_MISMATCH` carrying expected and actual when a masking script
reformats the input — read `actual` and adapt instead of re-firing blind.
