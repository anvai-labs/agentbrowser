...
| 2: shared application surfaces | AgentBrowser API/protocol/SDK/MCP/operator UI | Two separate bridge processes attach to one session; UI/API operation parity; one write wins; wrong tenant, stale grant and wrong resource produce zero effects. |
