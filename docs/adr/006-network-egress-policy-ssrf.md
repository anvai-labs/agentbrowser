...
- **Service workers are blocked by default whenever this policy is attached** (see [ADR-019](019-explicit-service-worker-opt-in.md), added 2026-09-13): `context.route()` cannot intercept service-worker-originated requests (a still-open Playwright limitation), so an unblocked service worker would bypass this choke point entirely. `serviceWorkers: 'block'` closes that hole but breaks any destination whose own behavior depends on a real service worker (e.g. anti-fraud tooling that treats a missing service worker as a bot signal — confirmed live against a real Ashby ATS submission). `allowServiceWorkers` is an explicit, off-by-default per-session opt-out a caller takes to accept that trade-off for one session; see ADR-019 for the full reasoning and consequences.
...
**As shipped** (`packages/engine-playwright/src/index.ts`, `installEgress()`): the request-interception path was taken, not a real proxy — `context.route('**', ...)`, checking the target via `checkRequest`/`checkResolvedAddresses`, then (for `GET`/`HEAD`/other non-body methods) `route.fetch({ maxRedirects: 0 })` followed by `route.fulfill({ response })` so redirect hops and response size can be checked before the page sees the response. As of 2026-09-12, `POST`/`PUT`/`PATCH` requests take the same target-verdict check but then call `route.continue()` instead of fetch/fulfill — see "Body-bearing requests bypass response inspection" under Consequences above for why. As of 2026-09-13 (ADR-019), the context is also created with `serviceWorkers: 'block'` whenever a request policy is attached, unless the session explicitly sets `allowServiceWorkers: true` — see "Service workers are blocked by default whenever this policy is attached" above.
