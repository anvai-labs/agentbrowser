...
for (const targetKind of ['button', 'formControls'] as const) {
  for (const transport of ['in-process', 'stdio', 'victor'] as const) {
    it.skipIf(transport === 'victor' && !process.env.AGENTBROWSER_VICTOR_ROOT)(
      `shares the operator panel and real Chromium through ${transport} MCP with independent ${targetKind} effects`,
      async () => {
        ...
        const fixture = createServer((req, res) => {
          ...
          res.end(
            `<!doctype html><title>Coexistence fixture</title><label>Note <input id="note"></label>${
              targetKind === 'button'
                ? '<button id="add">Add item</button>'
                : '<div id="add" data-automation-id="add-item">Add item</div>'
            }<script>add.onclick = e => fetch("/increment",{method:"POST",body:JSON.stringify({trusted:e.isTrusted,value:note.value})});</script>`
          );
        });
