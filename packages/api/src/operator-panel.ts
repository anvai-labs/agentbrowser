...
(() => {
  ...
  handle('delegate', async () => {
    const result = await api(path('/control/delegate'), 'POST', { epoch: review.epoch });
    state = { state: result.state, epoch: result.epoch, busy: result.busy };
    byId('grant').value = result.token; review = null;
    message('Transfer this session token to the harness environment. Keep your operator key private. Takeover immediately revokes the token.');
  });
...
const style =
  'body{font:16px system-ui;max-width:850px;margin:2rem auto;padding:1rem;color:#172536;background:#f5f7fa}label{display:block;margin-top:1rem}input{display:block;width:95%;padding:.6rem}button{padding:.6rem 1rem;margin:.6rem .4rem .6rem 0;cursor:pointer}button:disabled{cursor:default}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:white;padding:1rem;border:1px solid #c8d0da}#message{color:#8c2900}h1{font-size:1.7rem}';
...
<button id="takeover">Take over</button><button id="review">Prepare fresh review</button><button id="delegate">Delegate reviewed state</button><button id="stop">Stop session</button>
