/** Build first. Headed, disposable operator profile; --live also observes Yahoo. */
...
const require = createRequire(new URL('../packages/engine-playwright/package.json', import.meta.url));
const { chromium } = require('playwright');
const root = fileURLToPath(new URL('../', import.meta.url));
const version = JSON.parse(await readFile(new URL('../package.json', import.meta.url))).version;
const live = process.argv.includes('--live');
assert.ok(process.argv.slice(2).every(arg => arg === '--live'));
const profile = await mkdtemp(join(tmpdir(), 'agentbrowser-cdp-smoke-'));
const chrome = spawn(process.env.AGENTBROWSER_CHROME_PATH ?? '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', [
...
let observer, server, disabled;
const fixture = createServer((request, response) => {
  response.setHeader('Content-Type', 'text/html');
  response.end(`<title>CDP profile fixture</title><button>${request.headers.cookie?.includes('operator-marker=shared') ? 'Shared profile confirmed' : 'No profile marker'}</button>`);
});
try {
  let port;
  for (let attempt = 0; attempt < 150; attempt++) {
    ...
    if (/^[0-9]+\n/.test(active)) { port = Number(active.split('\n')[0]); break; }
    await delay(100);
  }
  ...
  server = await buildServer({
    engine: new PlaywrightChromiumEngine({ operatorCdp }), operatorCdp, apiKeys,
  ...
  const cli = async (...args) => {
    const result = await runExecutable([process.execPath, join(root, 'packages/cli/dist/bin.js'), '--base-url', baseUrl, '--timeout', '60000', '--json', ...args], { env, timeoutMs: 65000 });
  ...
  for (const view of [await client.sessions.get(session.sessionId), ...(await client.sessions.list())]) {
  ...
  const observation = await client.sessions.observe(session.sessionId, page.pageId, { mode: 'interactive' });
  assert.match(JSON.stringify(observation), /Shared profile confirmed/);
  await assert.rejects(client.sessions.cookies(session.sessionId), error => error.details?.reason === 'PROFILE_COOKIE_EXPORT_UNSUPPORTED');
  await assert.rejects(client.sessions.navigate(session.sessionId, page.pageId, { url: 'http://192.168.1.90/' }), error => error.details?.reason === 'egress_policy');
  ...
  const owned = context.pages().find(candidate => !existing.has(candidate) && candidate.url() === target && candidate !== sentinel);
  assert.ok(owned);
  await owned.evaluate(url => window.open(url), target);
  for (let attempt = 0; attempt < 30 && (await client.sessions.listPages(session.sessionId)).length < 2; attempt++) await delay(100);
  assert.equal((await client.sessions.listPages(session.sessionId)).length, 2);
  if (live) {
    const result = await cli('navigate', session.sessionId, page.pageId, 'https://finance.yahoo.com/quote/AAPL', '--wait-until', 'domcontentloaded');
    const observed = await client.sessions.observe(session.sessionId, page.pageId, { mode: 'interactive', maxBytes: 16000 });
    console.log(JSON.stringify({ live: 'Yahoo AAPL', status: result.status, title: observed.title ?? null }));
  ...
  await checkMcp([process.execPath, join(root, 'packages/mcp-server/dist/bin.js')], {
    expectedVersion: version, env, timeoutMs: 120000,
    async exercise({ request }) {
      const created = await request('tools/call', { name: 'browser_create', arguments: { tenantId: 'cdp-smoke', cdpAttach: true } });
  ...
  const refusal = await fetch(`${disabledUrl}/v1/sessions`, { method: 'POST', headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' }, body: JSON.stringify({ tenantId: 'cdp-smoke', cdpAttach: true }) });
  ...
  assert.equal(sentinel.isClosed(), false);
  console.log(JSON.stringify({ qualification: 'PASS', productVersion: version, browserVersion: observer.version(), endpointClass: 'loopback_http', profileSharing: true, unownedTabsPreserved: true, ownedPopupAdopted: true, detachPreservedBrowser: true, disabledRefusal: refused.error.details.reason }));
} finally {
